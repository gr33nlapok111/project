 ...
 */
if ( get_option( 'api_manager_affiliate_tools_activated' ) != 'Activated' ) {
    add_action( 'admin_notices', 'API_Manager_affiliate_tools::am_affiliate_tools_inactive_notice' );
}

class API_Manager_affiliate_tools {

	/**
	 * Self Upgrade Values
	 */
	// Base URL to the remote upgrade API Manager server. If not set then the Author URI is used.
	public $upgrade_url = 'http://starkexpressions.com';
	 ...
	 */
	public $api_manager_affiliate_tools_version_name = 'plugin_api_manager_affiliate_tools_version';
	 ...
	 */
	public $text_domain = 'affiliate-tools';
	...
	public $ame_update_check = 'am_affiliate_tools_plugin_update_check';
	...
	public function __construct() {
		...
		if ( is_admin() ) {
			 ...
			 */
			$this->ame_software_product_id = 'affiliate tools';

			/**
			 * Set all data defaults here
			 */
			$this->ame_data_key 				= 'api_manager_affiliate_tools';
			$this->ame_api_key 					= 'api_key';
			$this->ame_activation_email 		= 'activation_email';
			$this->ame_product_id_key 			= 'api_manager_affiliate_tools_product_id';
			$this->ame_instance_key 			= 'api_manager_affiliate_tools_instance';
			$this->ame_deactivate_checkbox_key 	= 'api_manager_affiliate_tools_deactivate_checkbox';
			$this->ame_activated_key 			= 'api_manager_affiliate_tools_activated';

			/**
			 * Set all admin menu data
			 */
			$this->ame_deactivate_checkbox 			= 'am_deactivate_affiliate_tools_checkbox';
			$this->ame_activation_tab_key 			= 'api_manager_affiliate_tools_dashboard';
			$this->ame_deactivation_tab_key 		= 'api_manager_affiliate_tools_deactivation';
			$this->ame_settings_menu_title 			= 'API Manager affiliate tools';
			$this->ame_settings_title 				= 'API Manager affiliate tools';
			$this->ame_menu_tab_activation_title 	= __( 'License Activation', 'api-manager-affiliate-tools' );
			$this->ame_menu_tab_deactivation_title 	= __( 'License Deactivation', 'api-manager-affiliate-tools' );
			...
			$this->ame_renew_license_url 	= 'http://starkexpressions.com/my-account'; // URL to renew a license. Trailing slash in the upgrade_url is required.
	 ...
	 * Update Check Class.
	 *
	 * @return API_Manager_affiliate_tools_Update_API_Check
	 */
	public function update_check( $upgrade_url, $plugin_name, $product_id, $api_key, $activation_email, $renew_license_url, $instance, $domain, $software_version, $plugin_or_theme, $text_domain, $extra = '' ) {
		...
		return API_Manager_affiliate_tools_Update_API_Check::instance( $upgrade_url, $plugin_name, $product_id, $api_key, $activation_email, $renew_license_url, $instance, $domain, $software_version, $plugin_or_theme, $text_domain, $extra );
	}
	...
	public function activation() {
		...
		$api_manager_affiliate_tools_password_management = new API_Manager_affiliate_tools_Password_Management();

		// Generate a unique installation $instance id
		$instance = $api_manager_affiliate_tools_password_management->generate_password( 12, false );
		...
		$curr_ver = get_option( $this->api_manager_affiliate_tools_version_name );

		// checks if the current plugin version is lower than the version being installed
		if ( version_compare( $this->version, $curr_ver, '>' ) ) {
			// update the version
			update_option( $this->api_manager_affiliate_tools_version_name, $this->version );
		}
     ...
     */
	public static function am_affiliate_tools_inactive_notice() { ?>
		<?php if ( ! current_user_can( 'manage_options' ) ) return; ?>
		<?php if ( isset( $_GET['page'] ) && 'api_manager_affiliate_tools_dashboard' == $_GET['page'] ) return; ?>
		<div id="message" class="error">
			<p><?php printf( __( 'The API Manager Affiliate Tools API License Key has not been activated, so the plugin is inactive! %sClick here%s to activate the license key and the plugin.', 'api-manager-affiliate-tools' ), '<a href="' . esc_url( admin_url( 'options-general.php?page=api_manager_affiliate_tools_dashboard' ) ) . '">', '</a>' ); ?></p>
		</div>
		<?php
	}
	...
	public function check_external_blocking() {
		...
		if( defined( 'WP_HTTP_BLOCK_EXTERNAL' ) && WP_HTTP_BLOCK_EXTERNAL === true ) {
			...
			if( ! defined( 'WP_ACCESSIBLE_HOSTS' ) || stristr( WP_ACCESSIBLE_HOSTS, $host ) === false ) {
				?>
				<div class="error">
					<p><?php printf( __( '<b>Warning!</b> You\'re blocking external requests which means you won\'t be able to get %s updates. Please add %s to %s.', 'api-manager-affiliate-tools' ), $this->ame_software_product_id, '<strong>' . $host . '</strong>', '<code>WP_ACCESSIBLE_HOSTS</code>'); ?></p>
				</div>
				<?php
			}
...
function AME() {
    return API_Manager_affiliate_tools::instance();
}
